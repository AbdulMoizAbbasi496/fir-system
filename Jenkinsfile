        ...
        always {
            script {
                ...
                try {
                    ...
                    echo "Could not get pusher email: ${err}"
                }
                ...
                try {
                    def summary = junit testResults: 'tests/target/surefire-reports/*.xml', allowEmptyResults: true
                    total = summary.totalCount
                    failed = summary.failCount
                    passed = total - failed
                } catch (err) {
                    echo "Could not read test results: ${err}"
                }
